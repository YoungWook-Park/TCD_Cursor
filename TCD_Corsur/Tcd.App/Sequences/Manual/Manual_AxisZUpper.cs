...
/// <summary>ZUpper축 수동 시퀀스 팩토리. 상부 챔버 Z축.</summary>
public static class Manual_AxisZUpper
{
    private const string Axis = AxisDefine.ZUpper;
    ...
    public static ISequence AbsMove() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_AbsMove, $"{Axis} AbsMove",
        async (ctx, p, ct) =>
        {
            CheckMotionInterlock(ctx);
            var core = MainCore.Instance;
            core.LogContext = new LogContext { SequenceKey = TcdSequenceKeys.Manual_Motor_ZUpper_AbsMove, RunId = Guid.NewGuid(), AxisName = Axis };
    ...
    public static ISequence IncMove() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_IncMove, $"{Axis} IncMove",
        async (ctx, p, ct) =>
        {
            CheckMotionInterlock(ctx);
            var core = MainCore.Instance;
            core.LogContext = new LogContext { SequenceKey = TcdSequenceKeys.Manual_Motor_ZUpper_IncMove, RunId = Guid.NewGuid(), AxisName = Axis };
    ...
    public static ISequence JogMove() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_JogMove, $"{Axis} JogMove",
        async (ctx, p, ct) =>
        {
            CheckMotionInterlock(ctx);
            var core = MainCore.Instance;
            core.LogContext = new LogContext { SequenceKey = TcdSequenceKeys.Manual_Motor_ZUpper_JogMove, RunId = Guid.NewGuid(), AxisName = Axis };
    ...
    public static ISequence Stop() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_Stop, $"{Axis} Stop",
        async (ctx, p, ct) =>
        {
    ...
    public static ISequence Home() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_Home, $"{Axis} Home",
        async (ctx, p, ct) =>
        {
            CheckMotionInterlock(ctx);
            // TODO: ZUpper Home 전용 인터락 조건 추가
            var core = MainCore.Instance;
            core.LogContext = new LogContext { SequenceKey = TcdSequenceKeys.Manual_Motor_ZUpper_Home, RunId = Guid.NewGuid(), AxisName = Axis };
    ...
    public static ISequence FaultReset() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_FaultReset, $"{Axis} FaultReset",
        async (ctx, p, ct) =>
        {
    ...
    public static ISequence ServoOn() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_ServoOn, $"{Axis} ServoOn",
        async (ctx, p, ct) =>
        {
    ...
    public static ISequence ServoOff() => new DelegateSequence(
        TcdSequenceKeys.Manual_Motor_ZUpper_ServoOff, $"{Axis} ServoOff",
        async (ctx, p, ct) =>
        {
