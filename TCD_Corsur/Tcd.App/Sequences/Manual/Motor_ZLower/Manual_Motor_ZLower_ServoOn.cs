...
/// <summary>ZLower축 서보 ON.</summary>
public sealed class ManualMotorZLowerServoOnSequence : ISequence
{
    #region Variables

    private const string AxisName = AxisDefine.ZLower;

    public string Key => TcdSequenceKeys.Manual_Motor_ZLower_ServoOn;
    public string DisplayName => "Manual Motor ZLower ServoOn";
    ...
    public async Task<SequenceResult> ExecuteAsync(ISequenceContext context, object parameter, CancellationToken cancellationToken)
    {
        ...
        try
        {
            core.Log.Info(core.LogContext, "Start", "ServoOn 시퀀스 시작");
            CheckInterlock(context);
            await core.Motion.ServoOnAsync(AxisName, cancellationToken).ConfigureAwait(false);
            core.Log.Info(core.LogContext, "End", "ServoOn 시퀀스 완료");
