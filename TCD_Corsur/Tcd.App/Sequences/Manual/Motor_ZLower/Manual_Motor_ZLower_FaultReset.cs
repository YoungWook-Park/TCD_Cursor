...
namespace Tcd.App.Sequences.Manual.Motor_ZLower;

/// <summary>ZLower축 결함 리셋.</summary>
public sealed class ManualMotorZLowerFaultResetSequence : ISequence
{
    #region Variables

    private const string AxisName = AxisDefine.ZLower;

    public string Key => TcdSequenceKeys.Manual_Motor_ZLower_FaultReset;
    public string DisplayName => "Manual Motor ZLower FaultReset";
    ...
    public async Task<SequenceResult> ExecuteAsync(ISequenceContext context, object parameter, CancellationToken cancellationToken)
    {
        ...
        try
        {
            core.Log.Info(core.LogContext, "Start", "FaultReset 시퀀스 시작");
            CheckInterlock(context);
            await core.Motion.FaultClearAsync(AxisName, cancellationToken).ConfigureAwait(false);
            core.Log.Info(core.LogContext, "End", "FaultReset 시퀀스 완료");
