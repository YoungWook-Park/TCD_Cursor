...
/// <summary>ZLower축 홈 복귀.</summary>
public sealed class ManualMotorZLowerHomeSequence : ISequence
{
    #region Variables

    private const string AxisName = AxisDefine.ZLower;

    public string Key => TcdSequenceKeys.Manual_Motor_ZLower_Home;
    public string DisplayName => "Manual Motor ZLower Home";
    ...
    public async Task<SequenceResult> ExecuteAsync(ISequenceContext context, object parameter, CancellationToken cancellationToken)
    {
        ...
        try
        {
            core.Log.Info(core.LogContext, "Start", "홈 시퀀스 시작");
            CheckInterlock(context);
            await core.Motion.HomeAsync(AxisName, cancellationToken).ConfigureAwait(false);
            core.Log.Info(core.LogContext, "End", "홈 시퀀스 완료");
