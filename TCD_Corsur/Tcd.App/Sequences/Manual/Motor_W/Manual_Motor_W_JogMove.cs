...
/// <summary>W축 조그. 파라미터는 속도(velocity, double, 부호가 방향). 취소 시까지 동작.</summary>
public sealed class ManualMotorWJogMoveSequence : ISequence
{
    #region Variables

    private const string AxisName = AxisDefine.W;

    public string Key => TcdSequenceKeys.Manual_Motor_W_JogMove;
    public string DisplayName => "Manual Motor W JogMove";
